...
import { ApiImage, components } from "@/lib/api"; // Import the components type from API
...
export function AnnotationProvider({ children }: { children: ReactNode }) {
  ...
  const saveCurrent = async (): Promise<void> => {
    try {
      ...
      const masksToSave: SaveMaskResponse[] = tabs
        .filter((tab) => tab.mask) // Only process tabs with masks
        .map((tab) => {
          // Convert the mask to base64 if needed
          const base64Src = tab.mask.src;
      ...
      if (masksToSave.length > 0) {
        ...
        console.log(
          `Saving ${masksToSave.length} masks for image ${currentImage.id}`
        );
